...
*/
static const uint8_t initial_state[256] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
...
__attribute__ ((pure)) bool rc4Match40b(const uint8_t *key, const uint8_t *bs, const uint8_t *match) {
    uint8_t state[256];
...
static void (*rc4d)(const uint8_t *key, const uint8_t *bs, const unsigned int len, uint8_t *out) = NULL;

/** Do 40-bit rc4-decrypt with key on bs of length len and
    put the result in out
*/
static void rc4Decrypt40b(const uint8_t *key, const uint8_t *bs, const unsigned int len, uint8_t *out) {
    uint8_t state[256];
    register unsigned int i;
    register uint8_t j, tmp;
...
/** Do 128-bit rc4-decrypt with key on bs of length len and
    put the result in out
*/
static void rc4Decrypt128b(const uint8_t *key, const uint8_t *bs, const unsigned int len, uint8_t *out) {
    uint8_t state[256];
    register int i;
    register uint8_t j, tmp;
...
static void rc4DecryptArb(const uint8_t *key, const uint8_t *bs, const unsigned int len, uint8_t *out) {
    uint8_t state[256];
    register unsigned int i;
    register uint8_t j, tmp;
...
inline void rc4Decrypt(const uint8_t *key, const uint8_t *bs, const unsigned int len, uint8_t *out) {
    rc4d(key, bs, len, out);
}

/** sets which function the wrapper should call */
__attribute__ ((pure)) bool setrc4DecryptMethod(const unsigned int length) {
    assert(length < 256);
    assert((length % 8) == 0);
