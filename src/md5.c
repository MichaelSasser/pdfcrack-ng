...
#define MD5_ROUND1(a, b, c, d, Xk, s, Ti)  a += (d ^ (b & (c ^ d))) + Xk + Ti; RnA(a,b,s)

/** MD5_ROUND2 optimized like above */
#define MD5_ROUND2(a, b, c, d, Xk, s, Ti) a += (c ^ (d & (b ^ c))) + Xk + Ti; RnA(a,b,s)
#define MD5_ROUND3(a, b, c, d, Xk, s, Ti) a += (b ^ c ^ d) + Xk + Ti; RnA(a,b,s)
#define MD5_ROUND4(a, b, c, d, Xk, s, Ti) a += (c ^ (b | ~d)) + Xk + Ti; RnA(a,b,s)
...
static void md5_50f(uint8_t *msg, const unsigned int msgLen __attribute__((unused))) {
    register uint32_t a, b, c, d;
    int i;
