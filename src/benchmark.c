...
/** print_and_clean was supposed to make the binary somewhat smaller but
    ...
    small as possible.
*/
static void print_and_clean(const char *str, unsigned int nrprocessed, const clock_t *start, const clock_t *end) {
    printf("%s\t%.1f\n", str, nrprocessed / (((double) (*end - *start)) / CLOCKS_PER_SEC));
    cleanPDFCrack();
    finished = false;
}

static void print_and_clean_wall_time(const char *str, unsigned int nrprocessed, struct timeval start,
                                      struct timeval end) {
    // pthread is NOT working with multiple threads
    // use gettimeofday() from sys/time.h instead for wall time measurements
    double diffSec = ((double) end.tv_sec - (double) start.tv_sec);
    double diffUSec = ((double) end.tv_usec - (double) start.tv_usec);
    diffSec += diffUSec / 1000000;
    printf("%s\t%.1f\n", str, (double) nrprocessed / diffSec);
...
static void pdf_40b_bench(int numThreads, const unsigned int numCpuCores) {
    clock_t startTime, endTime;
    ...
    initPDFCrack(&e, NULL, false, NULL, Generative, NULL, charset, 0, 5, true, numThreads, numCpuCores, 1, 1);
    ...
    initPDFCrack(&e, password, false, NULL, Generative, NULL, charset, 0, 5, true, numThreads, numCpuCores, 1, 1);

    startTime = clock();
    runCrackRev2_of();
    endTime = clock();

    print_and_clean("PDF (40, owner, fast):", getNrProcessed(), &startTime, &endTime);
}
