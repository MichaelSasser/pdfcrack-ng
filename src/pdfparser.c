...
static char *parseName(FILE *file) {
    ...
    ch = getc(file);
    for (i = 0; i < BUFFSIZE && !isWhiteSpace(ch) && !isDelimiter(ch) && ch != EOF; ++i) {
        buff[i] = (char) ch;
        ch = getc(file);
    }
    ungetc(ch, file);
...
__attribute__ ((pure)) static uint8_t hexToInt(const int b) {
    if (b >= '0' && b <= '9')
        return (uint8_t) (b - '0');
    else if (b >= 'a' && b <= 'f')
...
static p_str *parseHexString(const uint8_t *buf, const unsigned int len) {
    ...
    ret->len = (uint8_t) (len / 2);

    for (i = 0, j = 0; i < len; i += 2) {
        ret->content[j] = (uint8_t) (hexToInt(buf[i]) * 16);
